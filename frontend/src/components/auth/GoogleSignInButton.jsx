  ...
  useEffect(() => {
    ...
    const renderGsiButton = () => {
      ...
      try {
        ...
        // Clear previous render to prevent duplication
        if (googleBtnContainerRef.current) {
          googleBtnContainerRef.current.innerHTML = ''
        }

        // Render generic button inside container
    ...
    const tryInit = () => {
      if (window.google?.accounts?.id) {
        renderTimeout = setTimeout(renderGsiButton, 50)
      } else {
        let retries = 0
    ...
    <div className="w-full space-y-3">
      {loading ? (
        ...
        <div
          ...
          className="relative w-full h-[44px] flex items-center justify-center cursor-pointer group select-none"
        >
          {/* Visible Generic Google Sign-In Button (Always "Continue with Google" - Never personalized) */}
          <div className="w-full h-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-xl bg-white group-hover:bg-slate-50 text-slate-800 font-semibold text-sm transition-all border border-slate-300 shadow-sm active:scale-[0.99]">
          ...
          {/* Transparent Overlay for Native Google GIS Click Handling */}
          <div
            ref={googleBtnContainerRef}
            className="absolute inset-0 w-full h-full opacity-[0.001] overflow-hidden flex items-center justify-center cursor-pointer pointer-events-auto"
